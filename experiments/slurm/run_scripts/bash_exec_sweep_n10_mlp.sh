...
vf_coef_values=( 0.5 )
seed_values=( 8 42 6 )
policy_size_values=( small medium large )
num_files_values=( 10 )
...
python experiments/rl/ppo_w_cli_args.py --sweep-name=${sweep_name} --steer-disc=${steer_disc} --accel-disc=${accel_disc} --ent-coef=${ent_coef} --vf-coef=${vf_coef} --seed=${seed} --policy-size=${policy_size} --num-files=${num_files} --total-timesteps=${total_timesteps}
